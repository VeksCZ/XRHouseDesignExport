...
public class FloorPlanTests
{
    ...
    [Test]
    public void RoomSheet_DimensionLinesLieInsideTheRoom_ForBothWindings()
    {
        foreach (bool cw in new[] { false, true })
        {
            var room = Box("a", "Room", 0, 0, 4, 3, 0, cw);
            var dims = FloorPlanBuilder.BuildRoom(room).lines.Where(l => l.style == PlanStyle.Dimension).ToList();
            Assert.That(dims, Is.Not.Empty);
            foreach (var l in dims)
            {
                Vector2 mid = (l.a + l.b) / 2f;
                Assert.That(mid.x, Is.InRange(0f, 4f), $"dimension outside room (cw={cw}): {mid}");
                Assert.That(mid.y, Is.InRange(0f, 3f), $"dimension outside room (cw={cw}): {mid}");
            }
        }
    }
    ...
    public void Overview_DimensionsOnlyExteriorWallsFromOutside_AndSkipsTheSharedWall()
    {
        foreach (bool cw in new[] { false, true })
        {
            ...
            int dimLines = page.lines.Count(l => l.style == PlanStyle.Dimension);
            ...
            foreach (var l in page.lines.Where(l => l.style == PlanStyle.Dimension))
            {
    ...
    public void Levels_AreGroupedByFloorHeight()
    {
        ...
        Assert.That(FloorPlanBuilder.Flatten(plans).Count, Is.EqualTo(2 + 3));
    }
    ...
    public void Report_HasOneOverviewPerFloorAndOneSheetPerRoom()
    {
        ...
        Assert.That(System.Text.RegularExpressions.Regex.Matches(html, "<svg ").Count, Is.EqualTo(2 + 3));
    }
