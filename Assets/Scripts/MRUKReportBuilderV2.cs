...
public static class MRUKReportBuilderV2 {
    ...
    private static void DrawSvg(StringBuilder html, List<MRUKRoom> rooms, float correctionAngle = 0) {
        ...
        // 2. Process Walls with hole cutting (similar to 3D model logic but in 2D)
        foreach (var w in walls) {
            float wW = w.PlaneRect.Value.width;
            Vector3 wallPos = rot * w.transform.position;
            
            // Project the "right" vector to XZ plane
            ...
            if (wallHoles.Count > 0) {
                ...
                foreach (var h in wallHoles) {
                    ...
                    if (startX > currentX + 0.02f) {
                        // Add wall segment before the hole
